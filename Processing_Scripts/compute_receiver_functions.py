...
def compute_R_F(Data, Filt):
    ...
    filt = str(Filt)  # Define frequency range
    ...
    for i in range(len(stalist)):
        ...
        elif os.path.getsize(stalist[i]) < 150000:

            stadir = stalist[i].replace(direc+'/','')
            stadir = stalist[i].replace('/*.PICKLE','')

            if os.path.exists(direc + "<150KB/"+ stadir):
                shutil.move(stalist[i],direc + "<150KB/"+ stalist[i])
                print('File size less than 150 KB.')
            else:
                os.makedirs(direc + "<150KB/" + stadir)
                shutil.move(stalist[i],direc + "<150KB/"+ stalist[i])
                print('File size less than 150 KB.')
        else:
