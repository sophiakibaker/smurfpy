        ...
        if(len(stalist)>0):
            ...
            for i in range(len(stalist)): #range(cat.count()):
                    ...
                    if good:
                        ...
                        # plt.yticks(np.arange( np.min(RF/np.max(np.abs(RF))+np.round(distdg)), np.max(RF/np.max(np.abs(RF))+np.round(distdg)),5))
                        # plt.annotate(stalist[i],(RF/np.max(np.abs(RF))+np.round(distdg))[0])
