...
# Command line help
# if len(sys.argv) != 2 or str(Filt).lower() == 'help':
#     print('\n')
#     print('-----------------------------------------------------------------------------------------------------------------------')
#     print(sys.argv[0])
#     print('-----------------------------------------------------------------------------------------------------------------------')
...
def convert_to_depth(Data, noise, Model, Filt):
    ...
    if mod_1D == 'prem':
        taupmodel = TauPyModel(model='/Users/r03sb21/Documents/smurfpy/Tools/MODELS/PREM_FILES/prem_added_discon_taup.npz')
    elif mod_1D == 'ak135':
        #obspy.taup.taup_create.build_taup_model('./ak135_added_discon_taup.tvel','./ak135_added_discon_taup.npz')
        taupmodel = TauPyModel(model='/Users/r03sb21/Documents/smurfpy/Tools/MODELS/ak135_FILES/ak135_added_discon_taup.npz')


    def compute_onestation(dr,filter='Filt'):

        # Loop through receiver functions in station directory
        if os.path.isfile(dr + '/selected_RFs_'+noise+str(filter)+'.dat'):
            station_file = open(dr + '/selected_RFs_'+noise+str(filter)+'.dat','r')
            ...
            if not 'depths' in RFtrace:
                if seis[0].stats['dist']> 30.  and seis[0].stats['dist'] <90.:
                    ...
                    for d in range(1,len(discon)):
                        if mod_1D == 'prem':
                            taup_660=['taup_pierce -mod /Users/r03sb21/Documents/smurfpy/Tools/MODELS/PREM_FILES/prem_added_discon_taup.nd -h '+ str(depth) + ' -sta '+ str(slat)+' ' +str(slon)+ ' -evt '+ str(elat) + ' ' + str(elon) +' -ph '+discon[d] + ' -Pierce ' + str(x[d])+',0 -nodiscon']
                        if mod_1D == 'ak135':
                            taup_660=['taup_pierce -mod /Users/r03sb21/Documents/smurfpy/Tools/MODELS/ak135_FILES/ak135_added_discon_taup.tvel -h '+ str(depth) + ' -sta '+ str(slat)+' ' +str(slon)+ ' -evt '+ str(elat) + ' ' + str(elon) +' -ph '+discon[d] + ' -Pierce ' + str(x[d])+',0 -nodiscon'] #tvel
