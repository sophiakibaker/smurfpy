...
def auto_select_RF(Data, noise, Filt):
    # Command line help
...
#         sys.path.insert(0, '/Users/r03sb21/Documents/'+stadir)
        sys.path.insert(0, '/Volumes/GoogleDrive/My Drive/'+stadir)

        # if not os.path.exists(stadir + '/selected_RFs_' + noise + filt + '.dat'):
        #     os.makedirs(stadir + '/selected_RFs_' + noise + filt + '.dat')
        # if not os.path.exists(stadir + '/SNR_selected_RFs_' + noise + filt + '.dat'):
        #     os.makedirs(stadir + '/SNR_selected_RFs_' + noise + filt + '.dat')

        goodrffile = open(stadir+'/selected_RFs_' + noise + filt + '.dat', 'w')
        SNRrffile = open(stadir+'/SNR_selected_RFs_' + noise + filt + '.dat', 'w')
                                    ...
                                    if np.max(np.abs(RF.data[indm + 200:-1])) > minamp:
                                        ...
                                        if Sanne:
                                            ...
                                            if Noisemeasure > 2.5 and Noisemeasure2 > 2.0 and Noisemeasure2 < 100.:
                                                ...
                                                SNRrffile.write(stalist[i] + str(Noisemeasure) + str(Noisemeasure2) + '\n') # Print calculated SNR
                                        ...
                                        if Sophia:    
                                            ...
                                            # Test mean energy prior to P v.s. After P. on radial
                                            Pprior = SVref.slice(Ptime - 65., Ptime - 5.) # Select 1 min of data with safety gap
                                            Pprior_RMS = np.sqrt(np.sum(Pprior.data * Pprior.data) / float(len(Pprior.data)))                                        
                                            ...
                                            ####
                                            # Test mean energy between P and 75s vs after 75s on vertical
                                            Pless = Pref.slice(Ptime + 15., Ptime + 75.)
                                            Pless_RMS = np.sqrt(np.sum(Pless.data * Pless.data) / float(len(Pless.data)))                                        
                                            
                                            Pmore = Pref.slice(Ptime+75, Ptime + 175.)
                                            Pmore_RMS = np.sqrt(np.sum(Pmore.data * Pmore.data) / float(len(Pmore.data)))                                      
                                            
                                            SNR_V2 = Pmore_RMS / Pless_RMS
                                            
                                            # Test mean energy between P and 75s vs after 75s on radial
                                            Pless = SVref.slice(Ptime +15., Ptime + 75.) 
                                            Pless_RMS = np.sqrt(np.sum(Pless.data * Pless.data) / float(len(Pless.data)))                                        
                                            
                                            Pmore = SVref.slice(Ptime+75, Ptime + 175.) 
                                            Pmore_RMS = np.sqrt(np.sum(Pmore.data * Pmore.data) / float(len(Pmore.data)))                                      
                                            
                                            SNR_R2 = Pmore_RMS / Pless_RMS 
                                            ####                                       
                                            
                                            # Wavelength requirement

                                            # zero = np.sum(Pprior.data * Pprior.data) / 2*float(len(Pprior.data)) + np.sum(Ppost.data * Ppost.data) / 2*float(len(Ppost.data))
                                            # value = []
                                            # if vertical == zero:
                                            #     value.append(time)
                                            ...
                                            # set requirements for signal-to-noise
                                            # ratio THink 2.5/1.75 works well for African data
                                            if SNR_V2 <=1 and SNR_R2 <=1 and SNR_V >= 2.5 and SNR_R >= 1.75:
                                                print(i,stalist[i], 'passed selection')
                                                count = count + 1
    ...
    print("\\\\\    Total new good RFs for " + str(noise) + " is " + str(count) + '\n')
