...
#     print('\n')
#     sys.exit()
def plot_CCP(Data, noise, name, conv, filt, factor, coverage, type, params):

    Results = Data+'_Results'

    name = str(name)   # CCP_volume name
    conversion = str(conv)   # depth conversion used
    rffilter = str(filt)  # RF filter
    factor = float(factor)  #Fresnel Zone smoothing factor
    mincoverage = float(coverage) # Minimum data coverage (not always required)
    plot_type = str(type) # Determine the plot type:

    CCP = CCP_plot.ccp_volume()
    CCP.load_latest(
        Data, noise, name=name,
        filter=rffilter,
    ...
    if plot_type == 'COV':
        ...
        plot_depth=int(params)
        print('Plotting data coverage at '+ str(plot_depth) + 'km')
        CCP.plot_datacoverage(Data, plot_depth,name=name, conversion=conversion, filter=rffilter, factor=factor)
        plt.savefig(savepath + name + '_data_coverage_' + str(conversion) + '_' + noise + str(rffilter) + '_' + str(plot_depth) + '.pdf')
        plt.savefig(savepath + name + '_data_coverage_' + str(conversion) + '_' + noise + str(rffilter) + '_' + str(plot_depth) + '.png')
    ...
    if plot_type == 'TOPO':
        ...
        plt.savefig(savepath + name + '_discont_topo_' + str(conversion) + '_' + noise + str(rffilter) + '_' + str(plot_depth) + '.pdf')
        plt.savefig(savepath + name + '_discont_topo_' + str(conversion) + '_' + noise + str(rffilter) + '_' + str(plot_depth) + '.png')
...
# ------------------- MTZ Discontinuity topography ---------------- ####
    if plot_type == '3DTOPO':
        ...
        #     print('Try >> python3 plot_CCP.py help')
        #     sys.exit('Arguments incorrect.... exit')
        print('Plotting topography of discontinuities')
        CCP.topo3d(params,name=name, conversion=conversion, filter=rffilter, factor=factor, mincoverage=2., amplitude = False, blobs = False)
        plt.savefig(savepath + name + '_discont_topo3d_' + str(conversion) + '_' + noise + str(rffilter) + '.pdf')
        plt.savefig(savepath + name + '_discont_topo3d_' + str(conversion) + '_' + noise + str(rffilter) + '.png')
    
    if plot_type == '3DMTZ':
        CCP.mtzwidth3d(params,name=name, conversion=conversion, filter=rffilter, factor=factor, mincoverage=2., amplitude = False, blobs = False)
    # --------------------  MTZ thickness         ---------------- ####
    if plot_type == 'THICK':
        # if len(sys.argv) != 7:
        #     print('Try >> python3 plot_CCP.py help')
        ...
        CCP.plot_mtzwidth(name, conversion, rffilter, factor, mincoverage)
        plt.savefig(savepath + name + '_MTZ_thickness_' + str(conversion) + '_' + noise + str(rffilter) + '.pdf')
        plt.savefig(savepath + name + '_MTZ_thickness_' + str(conversion) + '_' + noise + str(rffilter) + '.png')
    ...
    if plot_type == 'GETTHICK':
        ...
        CCP.plot_mtzwidth_write(noise, name=name, conversion=conversion, filter=rffilter, factor=factor, mincoverage=2.)
    ...
    if plot_type == 'MOVEOUT':
        ...
        plt.savefig(savepath + name + '_moveout_plot_aligned_' + str(conversion) + '_' + noise + str(rffilter) + '_' + str(plot_depth) + '.pdf')
        plt.savefig(savepath + name + '_moveout_plot_aligned_' + str(conversion) + '_' + noise + str(rffilter) + '_' + str(plot_depth) + '.png')
    ...
    if plot_type == 'NS':
        ...
        plt.savefig(savepath + name + '_XC_' + str(plot_type) + '_at_' + str(lonorlat_seed) + 'Deg_' + str(conversion) + '_' + noise + str(rffilter) + '.pdf')
        plt.savefig(savepath + name + '_XC_' + str(plot_type) + '_at_' + str(lonorlat_seed) + 'Deg_' + str(conversion) + '_' + noise + str(rffilter) + '.png')
    ...
    if plot_type == 'XC':
        ...
        plt.savefig(savepath + name + '_XC_at_' +str(profile[1])+'N'+str(profile[0])+'E_to_' + str(profile[3])+'N'+str(profile[2])+'E_' + str(conversion) + '_' + noise + str(rffilter) + '.pdf')
        plt.savefig(savepath + name + '_XC_at_' +str(profile[1])+'N'+str(profile[0])+'E_to_' + str(profile[3])+'N'+str(profile[2])+'E_' + str(conversion) + '_' + noise + str(rffilter) + '.png')
