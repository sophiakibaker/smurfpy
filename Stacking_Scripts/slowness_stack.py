...
def slowness_stack(Data, noise, lonmin, lonmax, latmin, latmax, rffilter):
    ...
    savepath=Results+'/Slowness_Stacks'
    ...
    for direc in direcs:
        print(direc)
        file = direc + '/selected_RFs_' + noise + filt + '.dat'
    ...
    data = np.genfromtxt(pathtotools + 'Travel_Times_Slowness/TTS_Pds_'+str(epi_ref)+'.dat', delimiter='\t')
    ...
    plt.plot(x,y,label='Pds')
    # plt.annotate('Pds',xy=(130,-0.63))


    #---------------------Plot Predicted Travel Times and Slowness of PPvdp-----------------

    #Read in the predicted travel time and slowness differences from the appropriate file
    data = np.genfromtxt(pathtotools + 'Travel_Times_Slowness/TTS_PPvdp_'+str(epi_ref)+'.dat', delimiter='\t')
    ...
    #Read in the predicted travel time and slowness differences from the appropriate file
    data = np.genfromtxt(pathtotools + 'Travel_Times_Slowness/TTS_PPvds_'+str(epi_ref)+'.dat', delimiter='\t')
    ...
    #Read in the predicted travel time and slowness differences from the appropriate file
    datap = np.genfromtxt(pathtotools + 'Travel_Times_Slowness/TTS_PPvdp_'+str(epi_ref)+'.dat', delimiter='\t')
    #220km
    ...
    #Read in the predicted travel time and slowness differences from the appropriate file
    datas = np.genfromtxt(pathtotools + 'Travel_Times_Slowness/TTS_PPvds_'+str(epi_ref)+'.dat', delimiter='\t')
    #220km
    ...
    #Save figures
    plt.savefig(savepath+'/Figures/Slowness_'+noise+str(filt)+'_'+str(count_yes)+'RFs'+'.pdf')
    plt.savefig(savepath+'/Figures/Slowness_'+noise+str(filt)+'_'+str(count_yes)+'RFs'+'.png')
    ...
    output_file=str(savepath+'/Slowness_'+ noise +str(filt)+'_'+str(count_yes)+'RFs'+'.PICKLE')
